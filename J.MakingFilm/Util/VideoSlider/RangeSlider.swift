...
class RangeSliderThumbLayer: CALayer {
    ...
    override init() {
        super.init()
        self.imageButton = UIButton(frame: self.frame)
        self.addSublayer(imageButton!.layer)
    }
    ...
    override func draw(in ctx: CGContext) {
        ...
        imageButton?.isHighlighted = highlighted
    }
}
...
class RangeSlider: UIControl {
  ...
  @IBInspectable var lowerLayerSelected = Bool()
    ...
    var upperFrame: CGRect {
        return upperThumbLayer.frame
    }

    var lowerFrame: CGRect {
        return lowerThumbLayer.frame
    }
    ...
    func updateLayerFrames() {
        ...
        lowerThumbLayer.imageButton?.setImage(sliderLeftHandlerImage, for: .normal)
        lowerThumbLayer.imageButton?.setImage(sliderLeftHandlerSelImage, for: .highlighted)
        lowerThumbLayer.setNeedsDisplay()

        let upperThumbCenter = CGFloat(positionForValue(upperValue))
        upperThumbLayer.frame = CGRect(x: upperThumbCenter - thumbWidth/2.0, y: -5.0, width: thumbWidth, height: 50)
        upperThumbLayer.setNeedsDisplay()
        upperThumbLayer.imageButton?.setImage(sliderRightHandlerImage, for: .normal)
        upperThumbLayer.imageButton?.setImage(sliderRightHandlerSelImage, for: .highlighted)
