...
class VideoUtility: NSObject {
    ...
    func mergeAudioWithURL(videoUrl: URL, audioPath: String, savePath: URL, listener: ((Bool, URL) -> Void)?) {
        ...
        assetExport.exportAsynchronously {
            listener?(assetExport.status == .completed, savePath)
        }
    }
    ...
    func mergedVideo(videos: [AVAsset], saveFolder: String, videoTitle: String?, successListener: ((URL) -> Void)?, errorListener: ((Error?) -> Void)?) {
        ...
        var filename = String(format: "/J.MakingFilm_merged_video_%@.mp4", timeString)
        ...
        if FileManager.default.fileExists(atPath: path) {
            for content in try! FileManager.default.contentsOfDirectory(atPath: path) {
                do {
                    try FileManager.default.removeItem(at: URL(fileURLWithPath: path.appending("/\(content)")))
                    print("success remove file: \(path.appending("/\(content)"))")
                } catch {
                }
            }
        }
